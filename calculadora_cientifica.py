...
import timedef comb(n, r):
    return math.factorial(n) / (math.factorial(r) * math.factorial(n - r))class CalculadoraCientifica:
    def init(self, master):
        self.master = master
        master.title("BULLET Calculator")
        master.configure(bg='#2F4F4F')
        master.geometry("300x500")    self.shift_active = False
    ...
    new_width = 75
    new_height = int((new_width / float(width)) * height)  # Use float for division
    ...
    self.display = tk.Entry(self.display_frame, width=20, font=('Arial', 20), justify='right', bd=0, bg='#C0C0C0')  # Changed font to Arial as Digital-7 may not be standard
    ...
    botoes = [
        ...
        ('RCL', self.rcl, 'gray'), ('ENG', self.eng, 'gray'), ('(', self.parenthesis_open, 'gray'), (')', self.parenthesis_close, 'gray'), ('tan', self.tan, 'gray'),
    ...
    for i in range(8):  # Adjusted for rows
...
def calculate(self):
    try:
        # Pré-processar para modos de ângulo (simples, assume funções trig recebem valor diretamente)
        expr = self.current_expression.replace('math.sin', 'self._trig("sin"')
        expr = expr.replace('math.cos', 'self._trig("cos"')
        expr = expr.replace('math.tan', 'self._trig("tan"')
        expr = expr.replace('math.sinh', 'math.sinh')
        expr = expr.replace('math.cosh', 'math.cosh')
        expr = expr.replace('math.tanh', 'math.tanh')
        expr = expr.replace('comb', 'comb')  # Já definido global
        result = eval(expr, {"__builtins__": {}, "math": math, "cmath": cmath, "comb": comb, "self": self})
...
def _trig(self, func, value):
    if self.angle_mode == "DEG":
        value = math.radians(value)
    elif self.angle_mode == "GRAD":
        value = math.radians(value * 9.0 / 10.0)  # Grad to rad
    if func == "sin":
...
def mode(self):
    ...
    self.display.delete(0, tk.END)
    self.display.insert(tk.END, "Mode: " + self.angle_mode)
...
def fraction(self):
    try:
        ...
        self.current_expression = "%d/%d" % frac
        self.update_display()
    except:
        self.display.delete(0, tk.END)
        self.display.insert(tk.END, "Error")
...
def deg_min_sec(self):
    try:
        ...
        self.current_expression = "%d°%d'%f\"" % (degrees, minutes, seconds)
        self.update_display()
    except:
        self.display.delete(0, tk.END)
        self.display.insert(tk.END, "Error")
...
def hyp(self):
    if self.shift_active:
        modes = ["DEG", "RAD", "GRAD"]
        current_index = modes.index(self.angle_mode)
        self.angle_mode = modes[(current_index + 1) % 3]  # Cycle through all
        self.display.delete(0, tk.END)
        self.display.insert(tk.END, "Mode: " + self.angle_mode)
    else:
...
def decimal_to_fraction(self, x, error=0.000001):
    ...
    while True:
        ...
        if middle_d * (x + error) < middle_n:
            ...
            return (n * middle_d + middle_n, middle_d)root = tk.Tk()
